...
describe("verifyVtxo", () => {
    ...
    describe("path verification", () => {
        ...
        it("should verify a valid multi-hop path even when chain entries are out of order", async () => {
            ...
            const commitmentOutputScript = commitmentTx.getOutput(0)?.script;
            if (!commitmentOutputScript) {
                throw new Error("commitment tx missing output script");
            }
            parentTx.addInput({
                txid: hex.decode(commitmentTx.id),
                index: 0,
                witnessUtxo: {
                    script: commitmentOutputScript,
                    amount: 10_000n,
                },
...
async function buildCsvPathTx(opts: {
    ...
    tx.addInput({
        ...
        index: 0,
        witnessUtxo: {
            script: opts.parentOutputScript ?? vtxoScript.pkScript,
            amount: opts.amount,
        },
