...
export function verifyTreeSignatures(
    ...
    for (const subtree of tree.iterator()) {
        ...
        for (let i = 0; i < tx.inputsLength; i++) {
            ...
            // Required signer set MUST come from the script itself. Falling
            // back to whatever pubkeys the PSBT already signed with would let
            // any leaf with a self-consistent signature pass verification.
            ...
            try {
                ...
                results.push({
                    ...
                    valid: false,
                    signerKeys: [],
                    error: `Could not decode tapscript: ${errorMessage(err)}`,
                });
                continue;
            }
        ...
        for (let i = 0; i < tx.inputsLength; i++) {
            ...
            if (!internalKey) {
                results.push({
                    ...
                    inputIndex: i,
                    valid: false,
                    error: "Missing internal key in taproot input with tapLeafScript",
                });
                continue;
            }
