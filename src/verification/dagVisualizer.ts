...
export interface VtxoDAG {
    ...
    nodes: DAGNode[];
    /**
     * Edges connecting nodes. `from` is the parent (older, the tx whose
     * output is being spent); `to` is the child (newer, the tx that
     * spends it). So edges point parent → child, i.e. forward in time
     * along the virtual-tx history. Traversal for rendering the tree
     * from the VTXO back toward the commitment walks these edges in
     * reverse (match edges by `to === current` to find parents).
     */
    edges: DAGEdge[];
